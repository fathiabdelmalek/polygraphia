...
mod tests {
    ...
    fn test_affine_invalid_multipliers() {
        ...
        assert!(Affine::new(2, 5).is_err());
        assert!(Affine::new(4, 5).is_err());
        assert!(Affine::new(6, 5).is_err());
        assert!(Affine::new(8, 5).is_err());

        // 13 is a factor of 26
        assert!(Affine::new(13, 5).is_err());

        // 26 itself
        assert!(Affine::new(26, 5).is_err());
    }
    ...
    fn test_affine_valid_multipliers() {
        ...
        for &mult in &valid {
            assert!(Affine::new(mult, 0).is_ok(), "Multiplier {} should be valid", mult);
        }
    }
    ...
    fn test_affine_preserve_all_mode() {
        ...
        // Mixed content
        assert_eq!(cipher.encrypt("Test123!@#").unwrap(), "Nwin123!@#");
    }

    #[test]
    fn test_affine_alpha_only_mode() {
        let cipher = Affine::with_mode(5, 8, TextMode::AlphaOnly).unwrap();
    ...
    fn test_affine_case_preservation() {
        ...
        assert_eq!(cipher.encrypt("ABC").unwrap(), "IJK");
        assert_eq!(cipher.encrypt("abc").unwrap(), "ijk");
    }
    ...
    fn test_affine_set_multiplier() {
        let mut cipher = Affine::new(5, 8).unwrap();
        assert_eq!(cipher.encrypt("hello").unwrap(), "rclla");

        cipher.set_multiplier(7).unwrap();
        assert_eq!(cipher.encrypt("hello").unwrap(), "zebbi");
    ...
    fn test_affine_set_shift() {
        ...
        cipher.set_shift(3);
        assert_eq!(cipher.encrypt("hello").unwrap(), "mdggz");
    }
    ...
    fn test_affine_cipher_trait() {
        let cipher: Box<dyn Cipher> = Box::new(Affine::new(5, 8).unwrap());
    ...
    fn test_affine_modulo_behavior() {
        ...
        let cipher1 = Affine::new(5, 8).unwrap();
        let cipher2 = Affine::new(5, 34).unwrap(); // 34 % 26 = 8
